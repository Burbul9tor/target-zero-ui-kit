    ...
    <div ref="scroll" class="tz-table-new__scroll" tabindex="-1">
      ...
      <table class="tz-table-new__table" :class="{ 'is-stripped': stripped }" :style="{ minWidth: `${tableMinWidth}px` }">
        ...
        <thead v-if="showHeader"><tr>
          ...
          <th v-if="hasControl" class="settings-cell"><span class="settings-cell__inner"><button v-if="showSettings" type="button" class="icon-button" @click="settingsOpen = !settingsOpen; emit('settings')"><Settings :size="20" /></button></span></th>
        </tr></thead>
...
.selection-cell,.settings-cell,.action-cell{width:36px;min-width:36px;padding:0!important;text-align:center!important}.selection-cell .tz-checkbox,.action-cell .row-action{margin-inline:auto}.settings-cell__inner{display:block;width:100%;height:100%}.settings-cell .icon-button{position:absolute;top:50%;left:50%;margin:0;color:var(--brand-primary);line-height:0;transform:translate(-50%,-50%)}.settings-cell .icon-button svg{display:block}.row-action{color:var(--status-error-fg)}
