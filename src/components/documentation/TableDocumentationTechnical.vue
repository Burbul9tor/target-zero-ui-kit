...
const simplePanelFilters: Array<{ key: SimplePanelFilterKey; label: string; options: string[] }> = [
  { key: 'year', label: 'Год', options: years },
  { key: 'placeType', label: 'Тип места', options: placeTypes },
  { key: 'place', label: 'Место', options: places },
...
<template>
  <div class="table-docs">
    ...
    <section class="card wide">
      ...
      >
        <template #summary>
          <span class="panel-empty-summary">{{ panelActiveCount ? `Применено фильтров: ${panelActiveCount}` : 'Фильтры не применены' }}</span>
        </template>
        ...
        <div class="filter-launcher-list">
          <button type="button" class="filter-launcher" :class="{ active: draftObjects.length }" @click="simpleFilterOpen = null; objectModalOpen = true">
            ...
            <span><strong>Объект</strong></span>
...
.filter-launcher{display:grid;width:100%;min-height:48px;padding:var(--padding-spacing-8) var(--padding-spacing-10);grid-template-columns:auto minmax(0,1fr) auto auto;align-items:center;gap:var(--padding-spacing-8);color:var(--text-default);border:1px solid var(--border-default);border-radius:var(--radius-md);background:var(--bg-surface);font-family:var(--tz-font-family);text-align:left;cursor:pointer;transition:border-color 160ms ease,background-color 160ms ease,box-shadow 160ms ease}
