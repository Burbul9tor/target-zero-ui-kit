...
function addRow() {
  ...
  rows.value.unshift({ id, name: `Новый сотрудник · ${id}`, role: 'Тестовая запись', department: departments[0], object: objects[0] })
  page.value = 0
}
  ...
  <div class="table-docs">
    ...
    <section class="card wide">
      ...
      >
        ...
        <template #bar-right><button class="primary-button" type="button" @click="addRow"><Plus :size="16" /> Добавить</button></template>
    ...
    <section class="card">
      ...
      <TzTable :data="[]" :columns="columns" :columns-per-view="4" disabled-selection show-settings show-pagination />
    </section>
  </div>
</template>
