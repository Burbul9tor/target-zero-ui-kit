...
const controlType = ref<SegmentedValue>('manual')
const workflowStatus = ref<SegmentedValue>('draft')
...
const controlTypeItems: SegmentedItem[] = [
  { value: 'manual', label: 'Ручное управление' }, { value: 'auto', label: 'Автоматическое управление' },
]
const workflowStatusItems: SegmentedItem[] = [
  { value: 'draft', label: 'Черновик' }, { value: 'progress', label: 'В процессе' }, { value: 'done', label: 'Завершена' },
]
...
const api = [
  ...
  ['size', 'small | medium', 'Small — 24px; Medium — 32px по макету'],
  ...
  <div class="segmented-docs">
    ...
    <section class="card reference-card">
      ...
      <div class="reference-stage">
        <TzSegmentedControl v-model="health" label="Статус" :items="healthItems" size="medium" />
      </div>
    </section>

    <section class="card">
      <header><h2>Размеры</h2><p>По актуальному макету: Medium — 32px, Small — 24px. В обоих размерах используются текст 14/20, горизонтальные отступы 16px и радиус 6px.</p></header>
      <div class="compact-stage">
        <div class="size-example"><small>MEDIUM · 32PX</small><TzSegmentedControl v-model="compactValue" :items="compactItems" size="medium" aria-label="Выбор значения" /></div>
        <div class="size-example"><small>SMALL · 24PX</small><TzSegmentedControl v-model="compactValue" :items="compactItems" size="small" aria-label="Компактный выбор значения" /></div>
      </div>
    </section>

    <section class="card usage-example-card">
      <header><h2>Пример использования</h2><p>Сегменты располагаются отдельно с промежутком 4px. Выбранный вариант имеет зелёную заливку и белый текст.</p></header>
      <div class="figma-examples">
        <TzSegmentedControl v-model="compactValue" label="Вес вопроса (Максимальный балл)" :items="compactItems" size="small" />
        <TzSegmentedControl v-model="controlType" label="Тип управления *" :items="controlTypeItems" size="medium" />
        <TzSegmentedControl v-model="workflowStatus" label="Статус" :items="workflowStatusItems" size="medium" />
      </div>
    </section>
...
.compact-stage{display:flex;min-height:96px;padding:var(--padding-spacing-20);flex-wrap:wrap;align-items:flex-end;gap:var(--padding-spacing-32);border:1px solid var(--border-default);border-radius:var(--radius-md);background:var(--bg-page)}.size-example{display:flex;flex-direction:column;align-items:flex-start;gap:var(--padding-spacing-4)}.size-example small{color:var(--text-muted);font:500 10px/14px var(--tz-font-family)}.figma-examples{display:flex;flex-direction:column;align-items:flex-start;gap:var(--padding-spacing-20);padding:var(--padding-spacing-24);border:1px solid var(--border-default);border-radius:var(--radius-lg);background:var(--bg-page)}
